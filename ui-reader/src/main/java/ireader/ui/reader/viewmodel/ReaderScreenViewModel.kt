...
import ireader.domain.use_cases.local.book_usecases.BookMarkChapterUseCase
import ireader.domain.use_cases.preferences.reader_preferences.ReaderPrefUseCases
import ireader.domain.use_cases.reader.ScreenAlwaysOn
import ireader.domain.use_cases.remote.RemoteUseCases
import ireader.domain.use_cases.translate.TranslationEnginesManager
...
class ReaderScreenViewModel(
    val getBookUseCases: ireader.domain.use_cases.local.LocalGetBookUseCases,
    ...
    data class Param(val sourceId:Long? , val chapterId: Long?, val bookId: Long?)

    companion object  {
        fun createParam(controller: Controller) : Param {
            return Param(controller.navBackStackEntry.arguments?.getLong(NavigationArgs.sourceId.name),controller.navBackStackEntry.arguments?.getLong(NavigationArgs.chapterId.name),controller.navBackStackEntry.arguments?.getLong(NavigationArgs.bookId.name))
        }
    }
