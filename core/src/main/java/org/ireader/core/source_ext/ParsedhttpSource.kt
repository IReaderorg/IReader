...
//        val query = filters.filter { it.name == "query" }
//        if (query.isNotEmpty() && query.first().value != null) {
//            client.call(searchRequest())
//            val request = client.get<String>(searchRequest(page,
//                query.first().value as String, filters)).parseHtml()
