            ...
            when (status) {
                PackageInstaller.STATUS_PENDING_USER_ACTION -> {
                    ...
                    if (confirmationIntent == null) {
                        ...
                        deferred.complete(InstallStep.Error(localizeHelper.localize() { xml ->
                            xml.fatalError
                        }))
                        return
                    }
                ...
                PackageInstaller.STATUS_FAILURE_ABORTED -> {
                    deferred.complete(InstallStep.Error(localizeHelper.localize() { xml ->
                        xml.installationErrorAborted
                    }))
                }

                PackageInstaller.STATUS_FAILURE_BLOCKED -> {
                    deferred.complete(InstallStep.Error(localizeHelper.localize() { xml ->
                        xml.installationErrorBlocked
                    }))
                }

                PackageInstaller.STATUS_FAILURE_CONFLICT -> {
                    deferred.complete(InstallStep.Error(localizeHelper.localize() { xml ->
                        xml.installationErrorConflicted
                    }))
                }

                PackageInstaller.STATUS_FAILURE_INCOMPATIBLE -> {
                    deferred.complete(InstallStep.Error(localizeHelper.localize() { xml ->
                        xml.installationErrorIncompatible
                    }))
                }

                PackageInstaller.STATUS_FAILURE_STORAGE -> {
                    deferred.complete(InstallStep.Error(localizeHelper.localize() { xml ->
                        xml.installationErrorStorage
                    }))
                }

                PackageInstaller.STATUS_FAILURE_INVALID -> {
                    deferred.complete(InstallStep.Error(localizeHelper.localize() { xml ->
                        xml.installationErrorInvalid
                    }))
                }
                    ...
                    deferred.complete(InstallStep.Error(localizeHelper.localize() { xml ->
                        xml.packageInstallerFailedToInstallPackages
                    } + " $status"))
                }
            }
        }
