...
private data class BackupProto(
    @ProtoNumber(1) val library: List<ireader.domain.usecases.backup.backup.legecy.twnetysep.LegacyBookProto> = emptyList(),
...
internal fun ByteArray.dumpTwentySepLegacyBackup(): Backup {
    return ProtoBuf.decodeFromByteArray<ireader.domain.usecases.backup.backup.legecy.twnetysep.LegacyBackupProto>(
        this
    ).toBackup()
}
...
private data class LegacyBookProto(
    ...
    @ProtoNumber(15) val chapters: List<ireader.domain.usecases.backup.backup.legecy.twnetysep.LegacyChapterProto> = emptyList(),
