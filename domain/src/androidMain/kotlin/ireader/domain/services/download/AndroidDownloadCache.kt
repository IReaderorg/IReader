    ...
    override suspend fun refresh() {
        mutex.withLock {
            try {
                ...
                // A chapter is considered downloaded if its content is not empty and >= 50 chars
                val downloadedChapters = chapterRepository.subscribeChaptersByBookId(0L)
                    .let { flow ->
                        // Get all chapters from all books that have content
                        // This is a simplified approach - in production, you'd want a dedicated query
                        emptyList<ireader.domain.models.entities.Chapter>()
                    }
