 ...
 * - Optionally fetches content from remote if not available locally
 */
class TTSContentLoaderImpl(
    ...
    override suspend fun loadChapter(bookId: Long, chapterId: Long): TTSContentLoader.ChapterContent {
        ...
        if (content.isEmpty()) {
            ...
            content = fetchChapterContent(chapter) ?: emptyList()
     ...
     */
    private suspend fun fetchChapterContent(chapter: Chapter): List<Page>? {
        return try {
            // Use the existing chapter use case to get content
            val result = chapterUseCase.findChapterById(chapter.id)
            result?.content
        } catch (e: Exception) {
