    ...
    enum class Type {
        ...
        companion object {
            fun name(type: Type): UiText {
                return when (type) {
                    Type.Default -> UiText.MStringResource(Res.string.system_default)
                    Type.ByName -> UiText.MStringResource(Res.string.by_name)
                    Type.BySource -> UiText.MStringResource(Res.string.by_source)
                    Type.ByChapterNumber -> UiText.MStringResource(Res.string.by_chapter_number)
                    Type.Bookmark -> UiText.MStringResource(Res.string.by_bookmark)
                    Type.DateFetched -> UiText.MStringResource(Res.string.date_fetched)
                    Type.DateUpload -> UiText.MStringResource(Res.string.by_date_uploaded)
                    Type.Read -> UiText.MStringResource(Res.string.by_date_read)
                }
            }
        }
    }
