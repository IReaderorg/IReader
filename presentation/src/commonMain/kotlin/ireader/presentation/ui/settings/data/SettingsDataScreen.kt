...
private fun formatFileSize(bytes: Long): String {
    ...
    val kb = bytes / 1024.0
    if (kb < 1024) return "${ireader.domain.utils.toDecimalString(kb, 1)} KB"
    val mb = kb / 1024.0
    if (mb < 1024) return "${ireader.domain.utils.toDecimalString(mb, 1)} MB"
    val gb = mb / 1024.0
    return "${ireader.domain.utils.toDecimalString(gb, 1)} GB"
}
