...
internal class IosDatabaseHandler(
    val db: Database,
    ...
    private suspend fun <T> dispatch(inTransaction: Boolean, block: suspend Database.() -> T): T {
        ...
        // If we're currently in a transaction, no need to dispatch
        if (driver.currentTransaction() != null) {
            return block(db)
        }

        // Run on the query dispatcher
        return withContext(queryDispatcher) { block(db) }
    }

    /**
     * Simplified transaction handling for iOS
     * Uses SQLDelight's built-in transaction support
     */
